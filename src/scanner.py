...
# --- SURGICAL REPLACEMENT: Minimal Mock Classes instead of hardware import ---
class MockServo:
    angle = 0.0


class MockKit:
    servo = {1: MockServo(), 15: MockServo()}
...
def adapter_angle_to_width(
    ...
    width = min_width + (angle - min_angle) * (
        (max_width - min_width) / (max_angle - min_angle)
    )
    return width
...
def read_sensors(
    current_finger_angle: float, current_rotor_angle: float, a: float = 7.0
) -> tuple[float, float]:
    """
    ...
    target_finger_angle = min_angle + (clamped_width - min_width) * (
        (max_angle - min_angle) / (max_width - min_width)
    )
...
def scan_sequence(arm: Any, slice: int) -> list[tuple[float, float, str]]:
    ...
    for i in range(slice):
        ...
        while current_gripper_f_angle > closed_angle:
            ...
            if s1_out_of_bounds and s1_trigger_angle is None:
                ...
                print(
                    f"--> Sensor 1 Triggered at {current_gripper_f_angle:.1f}° (Value: {v1:.2f}V)"
                )

            if s2_out_of_bounds and s2_trigger_angle is None:
                s2_trigger_angle = current_gripper_f_angle
                print(
                    f"--> Sensor 2 Triggered at {current_gripper_f_angle:.1f}° (Value: {v2:.2f}V)"
                )
            ...
            if s1_trigger_angle is not None and s2_trigger_angle is not None:
                ...
                if current_step != slow_speed_step:
                    print(
                        "--> SLOWING DOWN: Single contact made, proceeding cautiously."
                    )
            ...
            # Sped up the simulated sleep slightly so the test runs faster
            time.sleep(0.005)
        ...
        if (
            s1_trigger_angle is not None
            and s2_trigger_angle is not None
            and s1_trigger_angle == s2_trigger_angle
        ):
